...
from . import wave_bridge_fragment_store as _fragment_store
from .wave_bridge_fragment_store import (
    _validate_fragment_store_value,
    _fragment_unpack_as,
    _fragment_store_unpack_element_type,
    _convert_fragment_store_value,
    _extract_converted_fragment_store_value,
    _extract_fragment_component,
    _mfma_fragment_store_vector_width,
    _mfma32_component_dim_exprs,
    _store_component_pointer_offset_expr,
    _store_component_expr,
    _mfma32_store_pointer_components_contiguous,
    _store_component_index_expr,
    _mfma32_store_component_slt_uniform,
    _mfma32_store_mask_compare_components_uniform,
    _mfma32_store_mask_components_uniform,
    _mfma_fragment_store_vector_width_for_store,
    _fragment_store_vector_width_candidates,
    _fragment_store_tile_component_dim_exprs,
    _fragment_store_tile_component_expr,
    _fragment_store_tile_pointer_components_contiguous,
    _fragment_store_tile_component_index_expr,
    _fragment_store_tile_mask_compare_components_uniform,
    _fragment_store_tile_mask_components_uniform,
    _fragment_store_tile_components_all_lanes_active,
    _fragment_store_tile_vector_width_for_store,
    _extract_fragment_store_value,
    _emit_component_store,
    _emit_masked_bounded_component_store,
    _emit_fragment_store,
    _emit_fragment_local_store,
    _fragment_store_tile_dim_bindings,
    _fragment_store_tile_component_all_lanes_active,
    _emit_mfma_fragment_tile_store,
    _emit_mfma_fragment_tile_local_store,
    _reject_unlowered_fragment_local_store_conversion,
    _physical_plan_is_mfma,
)
from . import wave_bridge_async as _async_copy
from .wave_bridge_async import (
    _dma_packet_layout_supported,
    _dma_packet_byte_candidates,
    _dma_packet_bytes,
    _dma_packet_elements,
    _dma_layout_width,
    _dma_layout_cta_threads,
    _log2_int,
    _linear_layout_static_coords,
    _validate_linear_dma_packet_layout,
    _validate_dma_packet_layout,
    _require_dma_packet_byte_candidates,
    _dma_packet_component_count,
    _dma_packet_dim_bindings,
    _dma_packet_uniform_dim_bindings,
    _offset_packet_dim_bindings,
    _ixsimpl_module,
    _ixsimpl_simplify,
    _ixsimpl_is_true,
    _ixsimpl_is_false,
    ...
    _ixsimpl_unknown_expr,
    _index_source_depends_on_dim,
    _mask_source_depends_on_dim,
    _ixsimpl_apply_index_binary,
    _ixsimpl_index_expr,
    _ixsimpl_index_packet_expr,
    _ixsimpl_pointer_offset_expr,
    _ixsimpl_pointer_packet_offset_expr,
    _ixsimpl_pointer_byte_offset_expr,
    _ixsimpl_pointer_packet_byte_offset_expr,
    _ixsimpl_shift_dim,
    _ixsimpl_mask_compare_expr,
    _ixsimpl_mask_expr,
    _dma_packet_dim_assumptions,
    _ixsimpl_assume_fact_exprs,
    _ixsimpl_index_source_proves_lower_bound,
    _ixsimpl_pointer_source_offset_proves_nonnegative,
    _dma_source_element_offset_proves_nonnegative,
    _require_dma_packet_source_contiguous_bytes,
    _require_dma_packet_mask_uniform,
    _ixsimpl_mod_zero,
    _ixsimpl_add_const,
    _ixsimpl_packet_slt_uniform,
    _ixsimpl_packet_mask_uniform,
    _emit_dma_packet_ptrs,
    _select_dma_packet_lowering,
    _zero_simd_for_element,
    _async_copy_other_value_plan,
    _async_copy_source_plan,
    _async_copy_pointer_source,
    _emit_async_copy_via_load_store,
    _emit_async_copy,
)
_WAVE_TOOL_NAMES = (
...
_GFX942_MMA16_INFO = _MmaShapeInfo(
...
_GFX942_SUPPORTED_MMA16_WARPS_PER_CTA = (2, 2)
_GFX942_SUPPORTED_MMA16_TILES_PER_WARP = (1, 1)
...
def _fragment_tile_shape_for_rank2_shape(
    shape,
    context,
    output_tile_shape=_GFX950_MMA_SHAPE,
):
    ...
    if shape[0] % output_tile_shape[0] or shape[1] % output_tile_shape[1]:
        raise ValueError(
            "tlx_wave bridge supports tiled MFMA fragments only when both "
            f"dimensions are multiples of {output_tile_shape}, got "
            f"shape={shape} while lowering {context}"
        )
    return (
        int(shape[0]) // int(output_tile_shape[0]),
        int(shape[1]) // int(output_tile_shape[1]),
    )
...
def _emit_constant_op(builder, op, values, wave_values, w, stats=None):
    ...
    if (
        ...
        mma = _mma_shape_for_result_value(
            value, "arith.constant #ttg.amd_mfma result"
        )
        tile_shape = _fragment_tile_shape_for_rank2_shape(
            value.shape,
            "arith.constant #ttg.amd_mfma result",
            mma.output_tile_shape,
        )
        zero = builder.constant(w.i32(), 0)
        acc_type = _acc_fragment_type(w, mma)
        fragments = tuple(
...
_async_copy.configure_async_copy_dependencies(globals())
...
def _mma_kind_for_element_type(element_type, mma):
    if element_type == "f16":
        if mma.instr_shape == (16, 16, 16):
            return _GFX942_F16_MMA_KIND
...
def _gfx942_mma16_shape_info(parent, context):
    if len(parent.warps_per_cta) != 2:
        raise ValueError(
            "tlx_wave bridge expected rank-2 gfx942/CDNA3 MFMA16 warpsPerCTA "
            f"while lowering {context}, got {parent.warps_per_cta}"
        )
    if parent.warps_per_cta != _GFX942_SUPPORTED_MMA16_WARPS_PER_CTA:
        derived_tile = (
            int(parent.instr_shape[0]) * int(parent.warps_per_cta[0]),
            int(parent.instr_shape[1]) * int(parent.warps_per_cta[1]),
        )
        raise ValueError(
            "tlx_wave bridge currently supports gfx942/CDNA3 MFMA16 only for "
            "the 32x32 CTA tile with warpsPerCTA="
            f"{_GFX942_SUPPORTED_MMA16_WARPS_PER_CTA}; "
            f"got warpsPerCTA={parent.warps_per_cta}, which implies CTA tile "
            f"{derived_tile} while lowering {context}"
        )
    tiles_per_warp = (
        _GFX942_SUPPORTED_MMA16_TILES_PER_WARP
        if parent.tiles_per_warp is None
        else parent.tiles_per_warp
    )
    if tiles_per_warp != _GFX942_SUPPORTED_MMA16_TILES_PER_WARP:
        raise ValueError(
            "tlx_wave bridge currently supports gfx942/CDNA3 MFMA16 only with "
            "unit tilesPerWarp; "
            f"got tilesPerWarp={tiles_per_warp} while lowering {context}"
        )
    return _GFX942_MMA16_INFO
...
def _dot_operand_mma_shape(info, context, attrs=None):
    ...
    if (
        parent.version == 3
        and parent.instr_shape == (16, 16, 16)
        and parent.is_transposed
    ):
        if attrs is not None and attrs.target != "hip:gfx942":
            raise ValueError(
                "tlx_wave bridge cannot lower gfx942 #ttg.amd_mfma parent to "
                f"Wave MFMA for TTGIR target {attrs.target}"
            )
        return _gfx942_mma16_shape_info(parent, context)
    raise ValueError(
        "tlx_wave bridge supports only gfx950 #ttg.amd_mfma<{version = 4, "
        "instrShape = [16, 16, 32] or [32, 32, 16], isTransposed = true}> "
        "or gfx942 #ttg.amd_mfma<{version = 3, instrShape = [16, 16, 16], "
        "isTransposed = true}> "
...
def _dot_operand_mma_kind(info, element_type, context, attrs=None):
    ...
    kind = _mma_kind_for_element_type(element_type, mma)
    if kind is None:
        raise ValueError(
...
def _validate_dot_op(operands, acc, result, attrs):
    ...
    mma_kind = _mma_kind_for_element_type(element_type, mma)
    if mma_kind is None:
        raise ValueError(
            "tlx_wave bridge supports only matching f16 x f16 or bf16 x bf16 "
            "tt.dot operands for Wave MFMA lowering; "
